...
import { UserStatus, usernameSchema, updateStatusSchema, updateCustomStatusSchema, updateStatusCommentSchema, toNamedPermissions, DEFAULT_AVATAR_LIMITS, isPreHashedPassword, updateActivitySchema, updateKeybindsSchema, NOISE_CANCELLATION_MODES } from '@sgchat/shared';
...
export const userRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.patch('/me/settings', {
    ...
    handler: async (request, _reply) => {
      ...
      if ('noise_cancellation_mode' in body) {
        if (!NOISE_CANCELLATION_MODES.includes(body.noise_cancellation_mode)) {
          return _reply.status(400).send({
            error: 'Invalid noise_cancellation_mode. Must be one of: off, native, nsnet2, deepfilter',
          });
        }
        // Sync legacy booleans for backward compat with older clients
        body.audio_noise_suppression = body.noise_cancellation_mode !== 'off';
        body.audio_ai_noise_suppression =
          body.noise_cancellation_mode === 'nsnet2' || body.noise_cancellation_mode === 'deepfilter';
      }
      if ('ns_aggressiveness' in body) {
        const val = Number(body.ns_aggressiveness);
        if (isNaN(val) || val < 0 || val > 1) {
          return _reply.status(400).send({
            error: 'ns_aggressiveness must be a number between 0 and 1',
          });
        }
        body.ns_aggressiveness = val;
      }
